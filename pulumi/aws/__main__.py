...
"""
    ...
    #
    # autoinstall can also pull in a newer AWS kernel package as a dependency
    # (e.g. on a g4dn AMI refresh) without the instance rebooting into it. If
    # that happens, the installed nvidia .ko only exists for the NEW kernel,
    # `nvidia-smi` fails against the still-running OLD one, and nvidia-ctk's
    # containerd config ends up pointing at a runtime that can never
    # initialize — k3s-agent then hangs forever on "Waiting for containerd
    # startup" and the node never joins the cluster. So: reboot
    # unconditionally right after the driver install (cheap even when no
    # kernel bump happened) and finish nvidia-container-toolkit + the k3s
    # agent join from a oneshot systemd unit on the NEXT boot, once whatever
    # driver got installed is actually loaded.
...
# Bucket survives teardown (protect=True). To delete it manually:
#   aws iam delete-user --user-name {user} (revoke access keys + inline policy first)
...
backup_user = aws.iam.User(
    ...
    tags=_tags(_backup_user_name, _teardown),
)
