...
```bash
docker run --rm --entrypoint age-keygen sinanozel/k3s-anywhere:0.2.2
```
...
docker run --rm -e ACTION=setup -e PROVIDER=aws -e AWS_REGION=us-east-1 \
  -v ~/.aws:/root/.aws:ro sinanozel/k3s-anywhere:0.2.2
...
docker run --rm -e ACTION=fetch --env-file scripts/fetch/.env \
  -v $(pwd)/output:/output sinanozel/k3s-anywhere:0.2.2
